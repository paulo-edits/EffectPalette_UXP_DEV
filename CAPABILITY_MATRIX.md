...
| Read video transition catalog | Tested in Premiere; full export pending retest | Available | None selected | Official factory returned 305 match names. Version 0.9.0 adds an explicit complete serializable export for comparison. The read-only QE-derived CEP file contained 340 visible video-transition names, so the catalogs are not assumed equivalent or positionally aligned. | 26.3.2 |
...
| Apply transition | Tested in Premiere; modern identity unresolved | Available | Existing stable product has routing | Empty/audio-only selections failed closed; start/end and multi-clip application succeeded with one Undo. Historical `ADBE Additive Dissolve` and `ADBE Film Dissolve` identifiers produced Legacy variants. Adobe confirms Film Impact replacements took over familiar names in 26.0, but UXP exposes neither transition display names nor readable `VideoTransition` identity. | 26.3.2 |
