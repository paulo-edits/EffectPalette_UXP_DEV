...
| Name Nest/subsequence | Partially tested in Premiere | Available | Existing stable product has routing | `ProjectItem.createSetNameAction()` correctly renamed the item in the Project panel. The nested Timeline item retained the generated `<source>_Sub_0X` name because official `Sequence.name` is read-only and `Sequence` exposes no rename action. | 26.3.2 |
