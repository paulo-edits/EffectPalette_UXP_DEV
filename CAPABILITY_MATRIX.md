...
| Replace selection with Nest | Unknown; not implemented | Available | Existing stable product has native routing | `Sequence.createSubsequence()` alone does not replace selected Timeline clips. A safe official remove-and-insert transaction must be designed and tested separately, including mixed audio/video and cross-track placement. | None |
