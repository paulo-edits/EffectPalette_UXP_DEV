...
| Replace selection with Nest | Tested in Premiere | Available | Existing stable product has native routing | Official-only probe created a root-level subsequence, removed the originals and inserted one nested item. Placement, serialized result and two-stage Undo behaved as expected. The Timeline item retained Premiere's generated internal sequence name; see naming limitation below. | 26.3.2 |
| Name Nest/subsequence | ProjectItem tested; TrackItem fix pending host test | Available | Existing stable product has routing | `ProjectItem.createSetNameAction()` correctly renames the Project-panel item. CEP reference showed that nested Timeline instances require a separate clip rename. Version 0.12.0 now applies official `VideoClipTrackItem`/`AudioClipTrackItem.createSetNameAction()` after insertion and reports every before/after name. | 26.3.2 |
