...
| Resolve video effect display-name/match-name pairs | Documented; implemented, pending host test | CEP normalizes display names from QE catalog | None selected | Explicit probe creates unattached components and reads their official display names. Reports every pair, failure and duration; does not modify the project. | None |
...
| Apply video effect | Tested in Premiere | Available through undocumented QE DOM | Existing stable product may use native routing | No selection and audio-only selection failed closed; one/multiple video clips succeeded with one Undo. `AE.ADBE Mosaic` resolved to Mosaic (Legacy), proving match-name identity must be paired with the component's resolved display name. | 26.3.2 |
