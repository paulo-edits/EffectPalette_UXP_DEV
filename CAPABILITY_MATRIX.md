...
| Insert project item into sequence | Implemented; host test pending | Available | Existing stable product has routing | Version 0.13.0 accepts exactly one Project-panel selection and performs official Insert or Overwrite at the playhead with explicit zero-based V/A track indices. Verification matches resulting TrackItems by ProjectItem ID and start ticks. Empty/multiple selection, invalid indices, media compatibility, track creation and Undo require host tests. | None |
