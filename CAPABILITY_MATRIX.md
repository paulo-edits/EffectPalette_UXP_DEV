...
| Resolve video effect display-name/match-name pairs | Tested in Premiere: unsupported before insertion; focused Impact research pending | CEP normalizes display names from QE catalog | None selected | `VideoFilterComponent` has no readable identity before insertion, and the two factory arrays have no documented positional mapping. The CEP catalog confirms modern and Legacy Mosaic coexist. The official probe now exports only Impact-related match names for evidence-based candidate testing. | 26.3.2 |
