...
| Create Nest/subsequence | Documented; implemented, pending host test | Available | Existing stable product has native routing | Version 0.10.0 requires explicit Timeline selection and calls `Sequence.createSubsequence(true)` to ignore targeting. It records creation count and resulting identity. Creation Undo behavior is unknown because the API returns a Sequence, not an Action. | None |
| Name Nest | Documented; implemented, pending host test | Available | Existing stable product has routing | The created sequence's ProjectItem is renamed with `createSetNameAction()` in a transaction. Whether both sequence and project-item visible names update together requires host confirmation. | None |
