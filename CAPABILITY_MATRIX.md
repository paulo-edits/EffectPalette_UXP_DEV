...
| Inventory all UXP-visible video effects | Locally generated from host evidence; confirmation incomplete | QE catalog provides independent visible names | None selected | All 829 UXP entries are present in `EFFECT_IDENTITY_MATRIX.json`; 691 display names also occur in the CEP snapshot. Two Mosaic pairs are confirmed and 827 same-index observations remain explicitly classified as runtime candidates because Adobe does not document positional pairing. | 26.3.2 snapshot |
