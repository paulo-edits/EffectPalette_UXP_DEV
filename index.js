...
async function createNestFromSelection(action) {
  ...
  // performNest also returns two live (non-JSON-serializable) object references - strip them
  // here, they can't cross the transport boundary.
  const { createdSequenceObject, insertedInstanceObjects, ...serializable } =
    await performNest(project, sourceSequence, requestedName, binName);
  return serializable;
}

// Extracted from createNestFromSelection's own body (which is now a thin wrapper above, unchanged
// behavior) when a second caller needed the same nest-creation mechanics. That caller - the Motion
// Tracker - has since moved to its own plugin, so this is again reached only through
// createNestFromSelection; the split is kept because it costs nothing and re-deriving nest creation
// from the API docs is exactly what this comment originally existed to prevent.
// Returns the same serializable diagnostic shape createNestFromSelection always returned, PLUS two
// live (non-serializable) object references - `createdSequenceObject` and `insertedInstanceObjects` -
// which that wrapper strips back out before handing the result to the transport (a live Premiere
// object can't cross that boundary).
async function performNest(project, sourceSequence, requestedName, binName) {
