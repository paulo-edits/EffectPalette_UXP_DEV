...
Premiere 26.3.2 host testing confirmed the complete replacement and Undo behavior. One official-API limitation remains: the requested name is applied to the ProjectItem in the Project panel, while the inserted Timeline item displays the generated internal sequence name (for example, `<source>_Sub_0X`). `Sequence.name` is read-only and the official `Sequence` API currently exposes no rename action, so the proof of concept does not use an undocumented workaround.
