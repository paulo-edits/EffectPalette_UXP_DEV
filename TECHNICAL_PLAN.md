...
## Slice: companion view-model extraction (UI rewrite, phase 1) — 2026-09-07/08

Branch `ui/qml-rewrite`. Preparation for rewriting the companion's view layer in QML (Qt Quick);
the design is in `docs/superpowers/specs/2026-09-07-companion-qml-ui-rewrite-design.md`, the plan in
`docs/superpowers/plans/2026-09-07-companion-view-model-extraction.md`.

### Why

The UI reads as amateur work and is painful to restyle: every window builds its own inline QSS blob
(`QtSettingsCenter`, `QtResultRowWidget`, `QtHotkeyCatalogEditor`, `QtDebugWindow` each carry a
different, drifted palette), the secondary windows are raw `QDialog`s next to a frameless palette,
and motion is hand-rolled `QPropertyAnimation` with magic durations. The user chose a QML rewrite
over a QtWidgets design-system pass because motion is the point. A non-Qt engine (WinUI 3 / web /
Flutter) was rejected: full rewrite of the Python and Win32 layers, and it would eliminate the
macOS option rather than preserve it.

### What landed (phase 1 only — no visual change by design)

State and logic moved out of `QtEffectPalette` into headless, testable objects; the QtWidgets views
stay and merely project them.

- **`companion/models.py`** — `MatchInfo`, `SearchResultSet`, `ResultRowModel`,
  `PaletteLayoutMetrics`, Qt-free. `app.py` re-exports them.
- **`companion/palette_view_models.py`** — `ResultsModel` (`QAbstractListModel`, camelCase
  `roleNames()` for QML), `PaletteViewModel` (query, category, selection, view state, status text,
  connection state, footer hint; pure helpers injected through the `QueryServices` protocol),
  `ApplyController` (idle → busy → success | error → idle).
- **`companion/window_control.py`** — `WindowAdapter` protocol, `WidgetWindowAdapter`,
  `PaletteWindowController`. The show/raise/activate + Win32 activation retry loop, previous-focus
  bookkeeping and pointer anchoring. Extracted against the existing widget window **first**, so the
  later `QuickWindowAdapter` swap is a one-class change.
- **App identity**: the exe had no icon resource, so Windows' "Installed apps" showed the generic
  glyph. Placeholder mark (`>_` prompt on a rounded indigo/violet tile) generated by
  `scripts/make_app_icon.py`, wired into the PyInstaller exe + version resource, the Inno installer
  and `UninstallDisplayIcon`, `app.setWindowIcon`, and the tray. Publisher corrected to
  `paulo.edits`. The user will replace the artwork later.

`QtEffectPalette` no longer owns `_active_category`, `_current_results`, `_current_row_models`,
`_current_result_set`, `_apply_busy`, `_apply_finishing`, `_apply_last_status`,
`_current_apply_effect`, `_apply_command_timestamp`, `_native_hwnd`, `_previous_foreground_hwnd`,
`_focus_attempt_job` or `_focus_reported`.

Three interfaces the plan had not anticipated: `ApplyController.set_command_timestamp` (the palette
paints itself busy before the command is sent), `ApplyController.set_last_status` (polling records
intermediate statuses to de-duplicate telemetry), and `PaletteWindowController.previous_handle` —
the native Nest, Label and recent-action paths read and write Premiere's handle from outside the
extracted methods.

### Evidence

Off-host: `npm run validate` passes; the companion suite went from 32 to 97 tests. The new
`test_window_control.py` covers the focus-retry loop, which previously had none.

**Host-tested (user's own session, 2026-09-08, Premiere Pro 26.3.2, project `TESTE MOTRACK`):**

| Check | Result |
| --- | --- |
| Palette opens | 6 |
| `palette_focus_acquired` | 6 / 6 |
| Focus attempt number | **0 on all six** (first try, no retries) |
| Focus latency | 31.8–48.1 ms, avg 37.6 |
| Hotkey backend | `native_windows` (`RegisterHotKey` still binds) |
| Preset apply | `Transform Smooth` completed `done` in 44.9 ms |

The five preceding `error_execution_failed` results in the same session were the wrong item
selected in Premiere, confirmed by the user — not a regression (the plugin side was not touched).
The refactored lifecycle handled them correctly: each failure re-enabled the input for a retry.

**Not verified in this session** — second-monitor placement, focus returning to Premiere after an
apply, native Nest and Timeline Label under the new `previous_handle` path. These still need a host
test before phase 2 merges.
